// supabaseService.js — Updated for Vite env vars (VITE_* instead of EXPO_PUBLIC_*)
...
export const saveHealthLog = async (userId, healthData) => {
  if (!supabase) return { error: 'Supabase not configured' };
  const { data, error } = await supabase
    .from('health_logs').insert({ user_id: userId, ...healthData }).select().single();
  return { data, error };
};
...
export const getHealthLogs = async (userId, limit = 30) => {
  ...
  const { data, error } = await supabase
    .from('health_logs').select('*').eq('user_id', userId)
...
export const saveEmergencyContact = async (userId, contact) => {
  if (!supabase) return { error: 'Supabase not configured' };
  const { data, error } = await supabase
    .from('emergency_contacts').upsert({ user_id: userId, ...contact }).select().single();
  return { data, error };
};
...
export const getEmergencyContacts = async (userId) => {
  if (!supabase) return { data: [], error: 'Supabase not configured' };
  const { data, error } = await supabase
    .from('emergency_contacts').select('*').eq('user_id', userId);
  return { data: data || [], error };
};
...
export const shareWithPartner = async (userId, partnerEmail) => {
  if (!supabase) return { error: 'Supabase not configured' };
  const { data, error } = await supabase
    .from('share_invites').insert({ owner_id: userId, partner_email: partnerEmail, status: 'pending' });
  return { data, error };
};
...
export const acceptShareInvite = async (userId, inviteId) => {
  if (!supabase) return { error: 'Supabase not configured' };
  const { data, error } = await supabase
    .from('share_invites').update({ partner_id: userId, status: 'accepted' })
...
export const getSharedProfiles = async (userId) => {
  ...
  const { data: invites, error } = await supabase
    .from('share_invites').select('*').eq('partner_id', userId).eq('status', 'accepted');
  if (error || !invites) return { data: [], error };
  const ownerIds = invites.map(i => i.owner_id);
  const { data: profiles, error: profileError } = await supabase
    .from('user_profiles').select('*').in('user_id', ownerIds);
...
export const subscribeToHealthUpdates = (userId, callback) => {
  ...
  return supabase
    ...
    .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'health_logs', filter: `user_id=eq.${userId}` },
